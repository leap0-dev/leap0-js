...
export function resolveConfig(input: Leap0ConfigInput = {}): Leap0ConfigResolved {
  ...
  const baseUrl = trimSlash((input.baseUrl ?? readEnv("LEAP0_BASE_URL") ?? DEFAULT_BASE_URL).trim())
  ...
  const sdkOtelEnabled =
    ...
    (envOtel === "true" ? true : envOtel === "false" ? false : Boolean(readEnv("OTEL_EXPORTER_OTLP_ENDPOINT")))
