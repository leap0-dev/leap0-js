...
test("accepts basic registry credentials", () => {
  const parsed = createTemplateParamsSchema.parse({
    ...
    credentials: { type: RegistryCredentialType.BASIC, username: "my-user", password: "my-password" },
  })
...
test("accepts AWS registry credentials", () => {
  const parsed = createTemplateParamsSchema.parse({
    ...
    credentials: { type: RegistryCredentialType.AWS, aws_access_key_id: "AKIA...", aws_secret_access_key: "secret", aws_region: "us-east-1" },
  })
...
test("accepts GCP registry credentials", () => {
  const parsed = createTemplateParamsSchema.parse({
    ...
    credentials: { type: RegistryCredentialType.GCP, gcp_service_account_json: '{"type":"service_account"}' },
  })
...
test("accepts Azure registry credentials", () => {
  const parsed = createTemplateParamsSchema.parse({
    ...
    credentials: { type: RegistryCredentialType.AZURE, azure_client_id: "client-id", azure_client_secret: "client-secret", azure_tenant_id: "tenant-id" },
  })
...
test("TemplatesClient validates credentials before transport", () => {
  let called = false
