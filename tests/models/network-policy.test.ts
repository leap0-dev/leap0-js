...
import { NetworkPolicyMode, createSandboxParamsSchema, networkPolicyModeSchema } from "@/models/sandbox.js"
...
test("createSandboxParamsSchema accepts network policy modes", () => {
  const allowAll = createSandboxParamsSchema.parse({ networkPolicy: { mode: NetworkPolicyMode.ALLOW_ALL } })
  const denyAll = createSandboxParamsSchema.parse({ networkPolicy: { mode: NetworkPolicyMode.DENY_ALL } })
