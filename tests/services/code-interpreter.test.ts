...
test("code interpreter client sends expected request shapes", async () => {
  ...
  await client.execute("sb-1", { code: "print(1)", language: "python" })
...
test("code interpreter stream parses SSE and maps event types", async () => {
  const { transport, calls } = createRecordedTransport({
    ...
    requestJsonUrl: () => Promise.reject(new Leap0Error("Code interpreter request failed")),
  })
  const client = new CodeInterpreterClient(transport as never, "sandbox.example.com")
  const events: Array<{ type: string; data: unknown }> = []
  for await (const event of client.executeStream("sb-1", { code: "print(1)", language: "python" })) events.push(event as { type: string; data: unknown })
  assert.deepEqual(events, [{ type: "stdout", data: "hello" }, { type: "exit", data: 0 }])
  assert.equal(calls[0]?.url, "https://sb-1.sandbox.example.com/execute/async")
  assert.deepEqual(jsonOf(calls[0]!), { code: "print(1)", language: "python" })
