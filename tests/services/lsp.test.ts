...
test("lsp client sends expected request shapes", async () => {
  ...
  assert.deepEqual(jsonOf(calls[2]!) as { uri: string }, { language_id: "typescript", path_to_project: "/workspace", uri: "file:///workspace/a.ts" })
  assert.deepEqual(jsonOf(calls[4]!), { language_id: "typescript", path_to_project: "/workspace", uri: "file:///workspace/a.ts", line: 1, character: 2 })
})
