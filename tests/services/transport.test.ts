...
import { Leap0ConflictError, Leap0Error, Leap0NotFoundError, Leap0PermissionError, Leap0RateLimitError, Leap0TimeoutError } from "@/core/errors.js"
import { Leap0Transport } from "@/core/transport.js"
import { installFetch } from "@tests/utils/helpers.ts"
import * as otel from "@opentelemetry/api"

function makeTransport() {
  return new Leap0Transport({ apiKey: "test-key", baseUrl: "https://api.example.com", sandboxDomain: "sandbox.example.com", timeout: 1, authHeader: "authorization", bearer: true, sdkOtelEnabled: false })
}
...
test("transport requestJson sends and parses JSON", async () => {
  ...
  try {
    const result = await makeTransport().requestJson("/v1/ok", { method: "POST", body: JSON.stringify({}) }, { query: { q: "x" } })
...
test("transport maps HTTP status codes to SDK errors", async () => {
  for (const [status, ErrorType] of [[403, Leap0PermissionError], [404, Leap0NotFoundError], [409, Leap0ConflictError], [429, Leap0RateLimitError], [500, Leap0Error]] as const) {
    const restore = installFetch(async () => new Response(JSON.stringify({ message: `status-${status}` }), { status }))
    try {
...
test("transport converts aborts to timeout errors", async () => {
  ...
  try {
    await assert.rejects(() => makeTransport().request("/v1/slow", {}, { timeout: 0.001 }), Leap0TimeoutError)
  } finally {
    restore()
  }
})
...
test("transport emits otel spans when enabled", async () => {
  ...
  otel.trace.getTracer = ((() => ({
    startActiveSpan(name: string, fn: (span: { setAttributes(attrs: Record<string, unknown>): void; setStatus(): void; recordException(): void; end(): void }) => Promise<Response>) {
      const span = {
        name,
        attributes: {},
        ended: false,
      ...
      spans.push(span)
      return fn({
        setAttributes(attrs) { span.attributes = attrs },
        setStatus() {},
        recordException() {},
        end() { span.ended = true },
      })
  ...
  const transport = new Leap0Transport({ apiKey: "test-key", baseUrl: "https://api.example.com", sandboxDomain: "sandbox.example.com", timeout: 1, authHeader: "authorization", bearer: true, sdkOtelEnabled: true })
  const restore = installFetch(async () => new Response(JSON.stringify({ ok: true }), { status: 200 }))
  try {
