...
test("client and sandbox helpers stay strongly typed", () => {
  ...
  expectTypeOf<Sandbox["filesystem"]["writeFile"]>().parameters.toEqualTypeOf<
    [path: string, content: string, options?: RequestOptions]
  >();
  expectTypeOf<Sandbox["git"]["clone"]>().parameters.toEqualTypeOf<
    [url: string, path: string, options?: RequestOptions]
  >();
  ...
  >();
  expectTypeOf<Sandbox["desktop"]["waitUntilReady"]>().parameters.toEqualTypeOf<
    [timeout?: number]
  >();
